...
之前用动态博客总是担心服务器忘了续费，或者程序升级出问题；接触了静态博客之后，感觉这是非常理想的解决方式，在Hugo、Hexo和Astro几个方案比较之后，最终选择了Astro.
...
1. **性能满分** - Google Lighthouse移动端和桌面端性能跑满100分。
2. **暗黑模式** - Logo旁边增加一个暗黑模式开关，纯css实现。
3. **缩略图 hover 放大** - 除了放大，点击图片可跳转到文章页。
4. **手机端不再加载预览图** - 控制不同屏幕尺寸加载不同资源，节省流量。
5. **阿里云oss处理图片** - 图片采用OSS处理，HTML渲染之后给图片添加参数，节省流量。
6. **性能优化** - 只有一个.js文件以处理导航栏滑动隐藏；CSS 通过 Vite 打包合并，图片懒加载。
